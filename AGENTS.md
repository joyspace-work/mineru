...
6. **精准币种识别与直接录入规则（无汇率折算）**：
   ...
   - **硬性限制**：在填入 `Official Suggested Price` 或三种 `Cost (EXW / FOB / FCA)` 价格前，你**必须**自动根据车源上下文识别其具体的计价货币，并**直接且仅填入**相对应的字段中。例如，若原始车源的指导价是人民币，则把相对应价格直接填入 `Official Suggested Price(CNY)`，而 `Official Suggested Price(USD)` 必须**留空为 `null`**。**绝对禁止 Agent 自己执行 any 汇率计算转换！** 原始车源没有提供的数据直接留空为 `null`。注意：**CIF 价格字段已废弃，不再收集，如遇到 CIF 报价请忽略。**
   - **最新硬性规则**：只要表头、价格列名、地点列或文本上下文出现 `EXW`、`FCA`、`FOB`，且没有明确写出 `人民币`、`RMB`、`CNY` 或 `¥`，就必须默认视为 USD 外贸价，分别写入 `cost_exw_usd`、`cost_fca_usd`、`cost_fob_usd`。严禁因为数值较大而自行除以汇率。
   - 如果 `EXW/FCA/FOB` 同时明确标注人民币，当前 `vehicle_sources` 没有独立人民币外贸价字段，应保留到 `supplier_price_cny` 和 `notes`，不得伪装写入 USD 字段。
   - 同一行同时出现 EXW/FCA/FOB 多个价格时，必须分别保留到对应字段，不能三选一。
