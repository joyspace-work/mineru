...
12. **取消外贸条款价格相互合理性与递增关系判断（忠实反映原数值）**：
    - **硬性规则**：不再对同一车源的多个外贸条款报价（`EXW` / `FCA` / `FOB`）做任何 `Cost (FOB) > Cost (FCA) > Cost (EXW)` 的比大小、互相纠偏或递增合理性推断。
    - **原则**：100% 忠实提取并保留源表格对应列中的实际数值。只要提取清洗精准即可，原表格单元格是多少就直接录入到对应的条款字段中，绝不根据倒挂或差价篡改原始价格。
